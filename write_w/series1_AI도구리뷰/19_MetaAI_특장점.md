...
Meta AI는 그 단계가 없다. 인스타그램 DM에도, 왓츠앱 채팅에도, 페이스북 검색창에도 이미 들어와 있다. 초인종 누른 적도 없는데 어느새 우리 집 거실에 앉아 있는 옆집 이웃 같은 포지션이다. 접근성 하나만 놓고 보면 이 시리즈에서 상대가 없다.

## 앱 하나가 아니라 바닥에 깔린 인프라

Meta AI의 존재감은 두 군데서 나온다. 하나는 방금 말한 도달 범위고, 다른 하나는 기반 모델인 Llama다.

Llama가 오픈웨이트로 공개되면서 전 세계의 개발자와 스타트업이 이걸 바탕으로 자기 서비스를 만들고 있다. 사용자 눈에 보이는 건 메신저 속 말풍선 하나지만, 그 아래에는 남의 제품을 떠받치는 기반 인프라 역할이 깔려 있다. 낮에는 옆집 이웃, 밤에는 동네 전체 수도관을 설계하는 엔지니어인 셈이다. 이 시리즈에서 앱과 인프라를 동시에 하는 캐릭터는 흔치 않다.
...
찾아갈 필요 없이 이미 옆에 와 있다. 존재감은 은근한데 도달 범위는 세계 최대급이다. 있는 줄도 몰랐는데 인스타 DM 한 번 잘못 눌러서 이미 대화 중이었다는 사람이 꽤 있다.
