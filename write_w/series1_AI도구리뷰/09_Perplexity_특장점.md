...
Perplexity는 그 질문을 미리 받아치는 쪽으로 설계됐다. 답을 주면서 "이건 여기서 가져왔어요"라며 링크를 같이 들이민다. 묻지도 않았는데 각주 번호를 붙이고, 심지어 그 각주가 진짜로 맞는 경우가 많다. 과제 낼 때 각주를 꼬박꼬박 다는 모범생 조교 같은 캐릭터다. "AI 검색엔진"이라는 카테고리를 사실상 대중화시킨 이름이기도 하다.
...
요약도 검색과 자연스럽게 붙어 있다. 여러 웹 출처를 동시에 훑어서 핵심만 압축해준다. 브라우저 탭 열 개 띄워놓고 하나씩 읽다가 8번째 탭쯤에서 처음 뭘 찾으려 했는지 잊어버리는 그 익숙한 코스를 통째로 건너뛸 수 있다.
...
묻는 말마다 참고문헌을 붙여주는 조교. 발표 전날 밤에 이 성실함의 가치를 알게 된다. "출처 어디예요?"라고 교수님이 물을까 봐 밤새 뒤척이던 사람에게는 거의 수면제급 안심이다.
