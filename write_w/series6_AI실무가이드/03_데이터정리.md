...
지저분한 데이터 파일을 하나 준비하자. 없으면 회사 어딘가에 반드시 있다. 대개 옆자리 동료의 바탕화면에 "최종진짜최종2"라는 이름으로 얌전히 앉아 있다.
...
1. 대표 이미지 — A person at a laptop looking at a messy spreadsheet full of inconsistent entries and stray colors, one hand on the mouse, a small duplicated file icon floating beside the screen as a backup. flat vector illustration style, someone working at a laptop with AI tools, minimal color palette, no text in the image, 16:9 composition
2. 첫 번째 이미지 자리 (원본 복사) — A single spreadsheet file icon being duplicated into two identical copies, one labeled with a small lock symbol to represent the untouched original. flat vector illustration style, someone working at a laptop with AI tools, minimal color palette, no text in the image, 4:3 composition
3. 두 번째 이미지 자리 (문제부터 진단) — A magnifying glass over a spreadsheet column showing the same city name spelled three different ways, with small warning dots marking inconsistent and messy cells. flat vector illustration style, someone working at a laptop with AI tools, minimal color palette, no text in the image, 4:3 composition
4. 세 번째 이미지 자리 (고치는 법을 받는다) — A person following numbered step-by-step instructions on a side panel while typing a formula directly into a single highlighted spreadsheet cell. flat vector illustration style, someone working at a laptop with AI tools, minimal color palette, no text in the image, 4:3 composition
5. 네 번째 이미지 자리 (검산) — A person comparing two spreadsheets side by side with a calculator, checking matching row counts and totals, a checkmark appearing between the two sheets. flat vector illustration style, someone working at a laptop with AI tools, minimal color palette, no text in the image, 4:3 composition
