# 05. 발표 자료에 넣을 이미지, 검색해서 가져다 쓰지 않고 30분에 만들기

> 소요 시간 30분 · 준비물 정리하고 싶은 사진 몇 장 · 난이도 ★★☆☆☆

---

발표 자료에 넣을 이미지가 필요한데 디자이너는 없고 포토샵은 못 쓴다. 이 상황에서 가장 흔한 선택은 인터넷에서 대충 검색해 가져다 쓰기인데, 회사에서 이보다 위험한 습관도 드물다. 저작권팀 메일은 생각보다 빨리 온다. 이 실습은 30분 안에 안전하고 쓸 만한 이미지를 확보하는 과정이다.
...
1. 대표 이미지 — A person at a laptop reviewing a grid of photo thumbnails, one photo with its background being cleanly cut away by a pair of floating scissors icon, other thumbnails showing a warning triangle to suggest copyright risk. flat vector illustration style, someone working at a laptop with AI tools, minimal color palette, no text in the image, 16:9 composition
2. 첫 번째 이미지 자리 (무엇이 필요한지 먼저 정한다) — Three labeled decision icons floating above a laptop screen — a ruler for size and ratio, an empty rectangle for text space, a shield with a checkmark for commercial-use rights. flat vector illustration style, someone working at a laptop with AI tools, minimal color palette, no text in the image, 4:3 composition
3. 두 번째 이미지 자리 (경로를 고른다) — Three diverging paths leading from a laptop — one to a camera and existing photo folder, one to a sparkle AI-generation icon, one to a free stock-photo globe icon, each path clearly separated. flat vector illustration style, someone working at a laptop with AI tools, minimal color palette, no text in the image, 4:3 composition
4. 세 번째 이미지 자리 (배경 제거와 정리) — A product photo on a laptop screen mid-transformation, messy background dissolving away on one half and a clean cropped subject remaining on the other half, small eraser and crop tool icons nearby. flat vector illustration style, someone working at a laptop with AI tools, minimal color palette, no text in the image, 4:3 composition
5. 네 번째 이미지 자리 (일관성을 만든다) — A row of several presentation slide thumbnails on a laptop screen, each image cropped to the same ratio and sharing the same warm color tone, a small palette icon tying them together. flat vector illustration style, someone working at a laptop with AI tools, minimal color palette, no text in the image, 4:3 composition
