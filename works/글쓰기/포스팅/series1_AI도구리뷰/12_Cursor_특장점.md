...
쓰려면 에디터를 갈아타야 한다. 십 년 넘게 손에 익은 단축키와 플러그인을 두고 나오라는 얘기다. 오래 만난 애인 버리고 소개팅 나가는 기분이랄까. 이 문턱에서 많은 사람이 한 번 멈춘다.
...
체감이 제일 큰 건 멀티파일 편집이다. "이 함수 이름 바꾸고 관련된 곳 다 고쳐줘"를 진짜로 끝까지 해낸다. 사람이었으면 "그거 어디어디서 썼는지 다 찾아야 하는데..."라며 한숨부터 쉬었을 일을, 토 달지 않고 파일 열두 개를 돌며 처리해놓는다. 여러 파일에 걸친 변경을 한 번에 계획하고 적용하기 때문에, 한 줄 채워주는 도구와는 작업 단위 자체가 다르다.
...
자동완성이 아니라 대신 짜준다. 그걸 겪고 나면 예전 에디터가 답답해진다는 말이 과장이 아니게 된다. 옛날 애인한테 연락 와도 이제는 심드렁해지는, 딱 그런 심리 상태가 된다.
