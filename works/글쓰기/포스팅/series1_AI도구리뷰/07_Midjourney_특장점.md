...
이 정도만 던져도 뭔가를 알아듣고 뽑아오는 쪽이 있다. 다른 도구라면 "죄송한데 조명은 어느 방향이고 인물은 몇 명이고..." 하며 되물었을 질문에, 이쪽은 눈빛만 보고 척척 그려오는 프리랜서 일러스트레이터다. 말수는 적은데 감각 하나는 기가 막힌다. Midjourney가 그 캐릭터다.
...
감으로만 찍는 도구도 아니다. `--style`, `--ar`, `--chaos` 같은 파라미터로 분위기를 꽤 정밀하게 조율할 수 있어서, 손에 익으면 원하는 톤앤매너를 겨냥해서 뽑아낼 수 있다. `--chaos` 숫자를 너무 올리면 프리랜서가 갑자기 예술혼에 각성해서 아무도 주문 안 한 그림을 그려오지만, 그것도 그것대로 가끔 대박이 난다. 처음엔 복불복 같다가 어느 순간부터 조종이 되는 종류의 도구다.

커뮤니티도 자산이다. 사용자들이 쌓아온 프롬프트와 결과물이 워낙 많아서, 특정 화풍이나 무드를 재현하는 노하우가 커뮤니티 차원에서 빠르게 돌고 쌓인다.

## 대신 글자는 못 쓴다

이미지 안에 텍스트를 정확히 넣어야 하는 작업, 그러니까 인포그래픽이나 문구가 들어간 배너라면 이쪽은 은근히 힘들어한다. 분명 "SALE"이라고 써달랬는데 미묘하게 다른 글자가 캔버스 어딘가에 새겨져 나오는 걸 보고 나면 다시는 안 시키게 된다. 배경만 지우거나 특정 오브젝트만 손보는 식의 정밀 편집도 주종목이 아니다. 그건 다른 도구가 할 일이다.
