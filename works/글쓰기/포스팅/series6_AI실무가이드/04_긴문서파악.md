# 04. 100페이지짜리 문서를 30분에 파악하는 법

> 소요 시간 30분 · 준비물 읽어야 하는데 미루고 있는 긴 문서 하나 · 난이도 ★★☆☆☆

---

책상 위에, 혹은 메일함 어딘가에 읽어야 하는데 하고 미뤄둔 긴 문서가 있을 것이다. 보고서, 논문, 사업계획서, 계약서, 매뉴얼 같은 것들. 어제도 열어보려다 다른 탭을 눌렀을 것이다. 이 실습은 그걸 30분 안에 남에게 설명할 수 있는 수준까지 끌어올리는 방법이다.
...
1. 대표 이미지 — A person at a laptop next to a tall stack of thick printed documents, one document open on the screen showing a clear outline structure instead of a wall of paragraphs. flat vector illustration style, someone working at a laptop with AI tools, minimal color palette, no text in the image, 16:9 composition
2. 첫 번째 이미지 자리 (구조 지도부터) — A long document transforming into a simple branching map or outline tree on a laptop screen, with one branch glowing brighter to show where to focus. flat vector illustration style, someone working at a laptop with AI tools, minimal color palette, no text in the image, 4:3 composition
3. 두 번째 이미지 자리 (주장과 근거 분리) — A laptop screen split into two columns, one column with bold claim icons and the other with supporting evidence icons, one claim column left empty with a warning flag. flat vector illustration style, someone working at a laptop with AI tools, minimal color palette, no text in the image, 4:3 composition
4. 세 번째 이미지 자리 (반대편에 서서 읽는다) — Two speech bubbles facing each other in debate, one presenting a document's argument and the opposite one raising a skeptical counter-question, person standing between them taking notes. flat vector illustration style, someone working at a laptop with AI tools, minimal color palette, no text in the image, 4:3 composition
5. 네 번째 이미지 자리 (30초 버전으로 압축) — A thick document being compressed step by step into three note cards of shrinking size, from a full page down to a single short sentence card. flat vector illustration style, someone working at a laptop with AI tools, minimal color palette, no text in the image, 4:3 composition
